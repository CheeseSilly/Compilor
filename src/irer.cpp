...
void rStatements(Node *node) {
  ...
  } else if (!strcmp(node->child[0]->name, "ReturnS")) {
    rReturnS(node->child[0]);
  }
}
...
void rCallS(Node *node) {
  ...
  if (!strcmp(node->child[0]->name, "CALL") &&
    ...
    Operand op = (Operand)malloc(sizeof(Operand_));
    Operand op0 = (Operand)malloc(sizeof(Operand_));
    op0->kind = Operand_::VARIABLE_OP;
    rExpr(node->child[3]->child[0]->child[0], op0);
    op->kind = Operand_::FUNCTION_OP;
    strcpy(op->u.value, node->child[1]->yytext);
    InterCode ir = (InterCode)malloc(sizeof(InterCode_));
...
void rReturnS(Node *node) {
  if (node == nullptr) {
    return;
  }
  if (!strcmp(node->child[0]->name, "RETURN") &&
      !strcmp(node->child[1]->name, "INTEGER_VAL")) {
    Operand op = new_constant(atoi(node->child[1]->yytext));
    InterCode ir = (InterCode)malloc(sizeof(InterCode_));
    ir->kind = InterCode_::RETURN_IR;
    ir->operands[0] = op;
    insertCode(ir);
  }
  if (!strcmp(node->child[0]->name, "RETURN") &&
      !strcmp(node->child[1]->name, "IDENTIFIER")) {
    FieldList f = allsearch(node->child[1]->yytext, 0);
    Operand op = (Operand)malloc(sizeof(Operand_));
    op->kind = Operand_::VARIABLE_OP;
    strcpy(op->u.value, f->name);
    InterCode ir = (InterCode)malloc(sizeof(InterCode_));
